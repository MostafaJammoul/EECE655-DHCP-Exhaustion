...
if __name__=="__main__":
    p=argparse.ArgumentParser(); p.add_argument("-i","--interface",default="eth0"); p.add_argument("-s","--start-ip",default="192.168.1.100"); p.add_argument("-e","--end-ip",default="192.168.1.110"); p.add_argument("--server-ip",default="192.168.1.1"); a=p.parse_args()
    iface,a.srv_ip = a.interface, a.server_ip
    if os.geteuid()!=0: print("[-] This script must be run as root!"); sys.exit(1)

    a1,*_,a4 = a.start_ip.split('.'); _,_,_,e = a.end_ip.split('.')
    pool = [f"{a1}.{_[0]}.{_[1]}.{i}" if False else None]  # dummy to keep compact style; replaced next
    # build pool properly:
    s = int(a.start_ip.split('.')[-1]); e = int(a.end_ip.split('.')[-1])
    base = '.'.join(a.start_ip.split('.')[:-1])
    pool = [f"{base}.{i}" for i in range(s, e+1)]

    available = pool.copy(); leases = {}; tx = {}
    ...
    def h(pkt):
        ...
        mac = pkt[Ether].src; xid = pkt[BOOTP].xid
        if mt==1:                    # DISCOVER
            if not available:
            ...
            if xid in tx:
                ...
                if m==mac and ip==rip:
                    if rip in available:
                        available.remove(rip); leases[mac]=rip
    ...
    try:
        ...
        if leases:
            print("\nLeased IPs:")
            for mac, ip in leases.items(): print(f"  {mac} -> {ip}")
